...
const Navbar = () => {
  ...
  // Function to check if a category should have its main link disabled
  const isMainLinkDisabled = (categoryName) => {
    return ["Hot Entertainment", "Music", "Traditional Art"].includes(categoryName);
  };
  ...
  const NavLink = ({ to, children, hasDropdown, category }) => (
    <div className="relative group">
      {isMainLinkDisabled(category) ? (
        <button
          className="flex items-center px-4 py-2 text-sm font-medium tracking-wide transition-all duration-300 rounded-lg group cursor-default"
          onClick={(e) => {
            e.preventDefault();
            if (hasDropdown) {
              toggleDropdown(category);
  ...
  const MobileNavLink = ({ to, children, hasDropdown, category }) => (
    <div className="border-b border-gray-100 last:border-0">
      <div className="flex items-center justify-between">
        {isMainLinkDisabled(category) ? (
          <div className="flex-1 px-4 py-3 text-sm font-medium text-gray-700 cursor-default">
            {children}
          </div>
        ) : (
          <Link
