...
const FormTraditionalArt = () => {
  ...
  const fetchArts = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/traditional-art/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await axios.put(`${API_URL}/api/traditional-art/${id}`, form, { headers });
      } else {
        await axios.post(`${API_URL}/api/traditional-art`, form, { headers });
      }
      ...
      <div className="max-w-4xl mx-auto bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl shadow-xl">
        <div className="p-6 mt-20">
          <h2 className="text-2xl md:text-3xl font-bold mb-6 text-white text-center">
            {id ? 'Edit Entertainment Item' : 'Add New Traditional Art Item'}
          </h2>
...
export default FormTraditionalArt;