const TradionalArt = require('../models/TraditionalArtModel');
...
exports.getAllArts = async (req, res) => {
    try {
        const art = await TradionalArt.find().sort({ createdAt: -1 });
        res.json(art);
    } catch (error) {
...
exports.getSingleArts = async (req, res) => {
    try {
        const art = await TradionalArt.findById(req.params.id);
        if (!art) {
...
exports.updateArts = async (req, res) => {
    try {
        ...
        if (req.file) {
            ...
            const oldArts = await TradionalArt.findById(req.params.id);
